MODULE ORG; (* DCWB 2017-01-16 code generator in Oberon-07 for x86 32 bit
  ...
  *)
  ...
  PROCEDURE SetOp*(op: INTEGER; VAR x, y: Item);   (* x := x op y *)
    VAR xset, yset: SYSTEM.SET32; (*x.type.form = Set*)
  BEGIN
    IF (x.mode = ORB.Const) & (y.mode = ORB.Const) THEN
      xset := SYSTEM.VAL(SYSTEM.SET32, x.a); yset := SYSTEM.VAL(SYSTEM.SET32, y.a);
  ...
  PROCEDURE Header*;
  BEGIN entry := pc*4;
    IF version = 0 THEN code[0] := 0FFFFFFFFE7000000H-1 + pc; Put1a(Mov, SB, 0, VarOrg0); Put1a(Mov, SP, 0, StkOrg0)  (*RISC-0*)
  ...
  BEGIN  (*exit code*)
    ...
    F := Files.New(name); Files.Set(R, F, 0); Files.WriteString(R, modid); Files.WriteInt(R, key); Files.Write(R, SHORT(SHORT(version)));
